...
def graph():
    ...
    log = allReadings(db, 'temperature', start=start, end=end)
    if len(log) == 0:
        return ('', 204)
...
def allReadings(db, table, start=None, end=None):
...
def latestReadings(db, table, minutes=None):
    ...
    for i in range(len(sensors)):
        rows = db.execute(
            'SELECT value, name FROM ' + table + ' t'
            ' JOIN sensor s ON t.sensor_id = s.id'
            ' WHERE name = ?'
            ' AND DATETIME(time, "localtime") > ?'
        ...
        if not (rows == None or len(rows) == 0):
            ...
            reading['value'] = round(rows[0]['value'])
